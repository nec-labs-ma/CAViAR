"""
Evaluation script for Cosmos-Reason2 (2B, 8B) on accident QA test set.

Uses native video input (file path + fps) as supported by the Cosmos-Reason2
processor, rather than individual frames.
...
Usage (base models):
  python evaluate_cosmos2.py --model 2B       # run with Cosmos-Reason2-2B
  python evaluate_cosmos2.py --model 8B       # run with Cosmos-Reason2-8B
  python evaluate_cosmos2.py --model all      # run both sequentially

Usage (finetuned models):
  python evaluate_cosmos2.py --model-path /path/to/finetuned_cosmos2
  python evaluate_cosmos2.py --model-path /path/to/finetuned --model-name my_ft_2B
"""
...
def strip_thinking(response_text):
    """Strip <think>...</think> reasoning block from Cosmos-Reason2 output.

    Returns the text after the closing </think> tag. If no thinking block is
    found, returns the original text.
    """
    match = re.search(r"</think>\s*", response_text)
    if match:
        return response_text[match.end():].strip()
    # Also handle case where <think> block is present but not closed
    match = re.search(r"<think>.*", response_text, re.DOTALL)
    if match:
        # Return whatever came before <think>, if anything
        before = response_text[:match.start()].strip()
        if before:
            return before
...
def run_evaluation(model_key, test_data, fps=DEFAULT_FPS, model_path=None):
    ...
    # Load model — try sdpa first (Cosmos2 default), then fallback
    try:
    ...
    for vid_idx, item in enumerate(tqdm(test_data, desc="Videos", unit="vid")):
        ...
        # Build native video content (Cosmos2 uses local path + fps)
        video_content = [
        ...
        for qa in target_qas:
            ...
            try:
                ...
                # Strip <think>...</think> reasoning to get the final answer
...
def save_results(results, model_label):
    ...
    output_path = os.path.join(OUTPUT_DIR, f"results_Cosmos2-{model_label}_4fps.json")
    ...
    if stats:
        print(f"\n  Multiple-Choice Accuracy (Cosmos2-{model_label}):")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Cosmos-Reason2 on accident QA test set")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        choices=["2B", "8B", "all"],
        help="Which base Cosmos-Reason2 model to evaluate (2B, 8B, or all)",
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default=None,
        help="Path to a finetuned Cosmos-Reason2 model (local directory). Overrides --model.",
    )
    parser.add_argument(
        "--model-name",
        type=str,
    ...
    if args.model_path:
        ...
        print(f"  Cosmos2-{model_label} evaluation took {elapsed:.1f}s")
        ...
        for model_key in models_to_run:
            ...
            elapsed = time.time() - start
            print(f"  Cosmos2-{model_key} evaluation took {elapsed:.1f}s")
