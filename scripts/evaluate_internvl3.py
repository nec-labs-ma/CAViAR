...
Usage (base models):
  python evaluate_internvl3.py --model 2B       # run with InternVL3-2B
  python evaluate_internvl3.py --model 8B       # run with InternVL3-8B
  python evaluate_internvl3.py --model 14B      # run with InternVL3-14B
  python evaluate_internvl3.py --model all      # run all three sequentially

Usage (finetuned models):
  python evaluate_internvl3.py --model-path /path/to/finetuned_internvl3_2B
  python evaluate_internvl3.py --model-path /path/to/finetuned --model-name my_ft_8B
"""
...
def get_frame_index(bound, fps, max_frame, first_idx=0, num_segments=32):
    if bound:
...
def load_video(video_path, bound=None, input_size=448, max_num=1, num_segments=32):
    """Load video frames and preprocess them for InternVL3."""
    vr = VideoReader(video_path, ctx=cpu(0), num_threads=1)
    max_frame = len(vr) - 1
    fps = float(vr.get_avg_fps())
    pixel_values_list, num_patches_list = [], []
    transform = build_transform(input_size=input_size)
    frame_indices = get_frame_index(bound, fps, max_frame, first_idx=0, num_segments=num_segments)
...
# Benchmark-specific system contexts for multiple-choice tasks
MC_CONTEXT = {
    "Weather & Light": (
...
# Benchmark-specific system contexts and answer instructions for open-ended tasks
OE_CONTEXT = {
...
def parse_mc_answer(response_text, choices):
    ...
    # Try to find a letter A-Z at the start
    for i, c in enumerate(choices):
        letter = chr(65 + i)
        if text.upper().startswith(letter):
            return c
    # Fallback: check if the response matches one of the choices
    for c in choices:
...
def run_evaluation(model_key, test_data, num_frames=16, model_path=None):
    ...
    try:
        ...
        # Finetuned models may not accept use_flash_attn directly
        model = AutoModel.from_pretrained(
            model_name,
            torch_dtype=torch.bfloat16,
            low_cpu_mem_usage=True,
    ...
    for vid_idx, item in enumerate(test_data):
        ...
        # Load video frames (max_num=1 per frame to keep memory manageable)
        try:
        ...
        # Build the video frame prefix
        video_prefix = ''.join([
            f'Frame{i + 1}: <image>\n' for i in range(len(num_patches_list))
        ])

        # Filter QA pairs for our target benchmarks
...
def save_results(results, model_label):
    ...
    output_path = os.path.join(OUTPUT_DIR, f"results_InternVL3-{model_label}_lora.json")

    with open(output_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\n  Results saved to: {output_path}")

    # Print MC accuracy summary
...
# ── Entry point ──────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Evaluate InternVL3 on accident QA test set")
    parser.add_argument(
